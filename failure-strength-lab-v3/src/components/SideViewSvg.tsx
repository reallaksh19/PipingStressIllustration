  ...
  return <g>
    ...
    {stages.map(stage => {
      ...
      return <g key={stage.key}>
        <circle cx={stage.x} cy="304" r={on ? 8 : 5} fill={on ? stage.color : 'rgba(216,237,255,.26)'} stroke="#06101d" strokeWidth="2" />
        <text x={stage.x} y="321" textAnchor="middle" fill={on ? stage.color : 'rgba(169,189,213,.86)'} fontSize="11" fontWeight="900">{stage.label}</text>
      </g>;
    })}
  </g>;
}
