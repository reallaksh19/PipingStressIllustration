...
type LearningPack = {
  tab: string;
  subtitle: string;
  helpers: LearningHelper[];
};

export function LearningCenterPanel({ state }: { state: LabState }) {
  const pack = learningPackFor(state);
  if (!pack) return null;

  return <article
    className="panel learning-center-panel"
    aria-label={`${pack.tab} learning center`}
    style={{ marginTop: 14 }}
  >
    <div className="ph">
    ...
    <div className="pb" style={{ display: 'grid', gap: 10 }}>
      <div style={{
        ...
        padding: '12px 14px',
        borderRadius: 18,
        border: '1px solid rgba(82,240,223,.26)',
        background: 'linear-gradient(135deg, rgba(82,240,223,.10), rgba(85,184,255,.06))',
      }}>
        <b style={{ color: '#dcfffb', fontSize: 13, letterSpacing: '.08em', textTransform: 'uppercase' }}>{pack.tab}</b>
        <span className="copy" style={{ lineHeight: 1.42 }}>{pack.subtitle}</span>
      </div>

      <div style={{ display: 'grid', gap: 9 }}>
        {pack.helpers.map((helper, index) => <details
          key={`${helper.title}-${helper.route}`}
          open={index === 0 || helper.title.includes('B31.3')}
          style={{
        ...
        >
          <summary style={{ cursor: 'pointer', padding: '10px 12px', color: '#d8edff', fontWeight: 950, fontSize: 13 }}>
            {helper.title} <span style={{ color: '#52f0df' }}>· {helper.route}</span>
          </summary>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 8, padding: '0 12px 12px' }}>
...
function LearningCell({ title, text, color }: { title: string; text: string; color: string }) {
  return <div style={{ minWidth: 0, display: 'grid', gap: 4, padding: '9px 10px', borderRadius: 13, border: '1px solid rgba(190,220,255,.12)', background: 'rgba(6,16,29,.46)' }}>
    <b style={{ color, fontSize: 10, letterSpacing: '.07em', textTransform: 'uppercase' }}>{title}</b>
    <span className="copy" style={{ fontSize: 12, lineHeight: 1.34 }}>{text}</span>
  </div>;
}

function learningPackFor(state: LabState): LearningPack | null {
  if (state.mode === 'static') {
    return {
      tab: 'Tab 1 · Static Loading',
      subtitle: 'Material response, static demand, pipe-wall behavior, stress-strain meaning, and B31.3 paragraph-route awareness.',
      helpers: staticLearningHelpers(state),
    };
  }
  if (state.mode === 'fatigue') {
    return {
      tab: 'Tab 2 · Fatigue Loading',
      subtitle: 'Stress range, cycles, hotspot detail, S-N limitations, and practical piping fatigue/B31.3 route awareness.',
      helpers: fatigueLearningHelpers(state),
    };
  }
  return null;
}
  ...
  return [
    {
      ...
      b313: 'Map to material suitability and allowable-stress basis: Chapter III / para. 323 family and Appendix A allowable tables. Verify exact material, temperature, service, and edition in the licensed code.',
      ...
      next: 'For pipe-specific components, go to Pipe Stress · σθ σL τ after understanding material response.',
    },
    {
    ...
    },
    {
      ...
      route: 'local wall',
      concept: 'The cross-section helper shows the pipe wall as an annulus. Static axial stress acts through metal wall area, while local yield/crush/fracture depends on material and geometry.',
