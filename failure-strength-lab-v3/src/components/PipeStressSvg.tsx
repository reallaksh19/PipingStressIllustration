...
export function PipeStressSideSvg({ state }: { state: LabState }) {
  ...
  return <svg viewBox="0 0 460 340" role="img" aria-label="Pipe stress side view using pipe notation">
    ...
    <text x="230" y="63" textAnchor="middle" className="muted">Cylindrical pipe notation: σL axial, σθ hoop, τ torsion</text>
...
export function PipeStressSectionSvg({ state }: { state: LabState }) {
  ...
  return <svg viewBox="0 0 460 340" role="img" aria-label="Pipe stress cross-section with hoop, ovalisation, torsion and rupture cues">
    ...
    <text x="230" y="63" textAnchor="middle" className="muted">Hoop/pressure, bending ovalisation, and torsion are separated here</text>
...
export function PipeStressReadout({ state }: { state: LabState }) {
  ...
  return <div className="interp stress-readout">
    <span className="badge" style={{ color: COLORS.cyan }}>pipe notation</span>
    <h3 className="result-title">5B Pipe stress components</h3>
    <p className="copy">This tab uses cylindrical pipe notation. It no longer maps generic σx/σy into pipe stress. Hoop, axial, bending, and torsion have their own sliders.</p>
    <div className="table">
      {pressureOn && <>
        <div><span>σθ</span><b>{pct(state.pipeHoop)} · hoop/circumferential stress from pressure</b></div>
        <div><span>σL</span><b>{pct(state.pipeAxial)} · longitudinal/axial membrane stress</b></div>
      </>}
      {bendingOn && <div><span>M bend</span><b>{pct(state.pipeBending)} · longitudinal tension/compression distribution; ovalisation cue</b></div>}
      {torsionOn && <div><span>τt</span><b>{pct(state.pipeTorsion)} · torsional shear; diagonal/helical band cue</b></div>}
      <div><span>View</span><b>{state.pipeStressView}</b></div>
    </div>
    <div className="bucket" style={{ borderColor: 'rgba(82,240,223,.28)' }}><b>Coordinate systems are separate</b><span className="copy">5A stays as generic Cartesian σx, σy, τxy. 5B is pipe-specific σθ, σL, σr and τ. Do not read σy as hoop unless a local pipe coordinate system has explicitly been defined.</span></div>
  </div>;
}
...
export function PipeStressNote({ state }: { state: LabState }) {
  ...
  return <div className="interp stress-readout">
    ...
    <h3 className="result-title">Clarified concepts before failure theory</h3>
    <div className="card correct"><strong>Hoop stress σθ</strong><span className="copy">Circumferential membrane stress around the pipe wall. In pressure-dominated rupture, high hoop stress tends to open a longitudinal split.</span></div>
    <div className="card correct"><strong>Axial stress σL</strong><span className="copy">Longitudinal membrane stress acts along the pipe axis. It changes the side-view length cue, not the cross-section hoop expansion.</span></div>
    <div className="card correct"><strong>Ovalisation</strong><span className="copy">Shown only under bending. Bending produces longitudinal tension on one side and compression on the opposite side; the circular cross-section can flatten.</span></div>
    <div className="card correct"><strong>Torsion shear τt</strong><span className="copy">Shown as diagonal/helical shear bands. It is not labelled as pressure rupture.</span></div>
    <p className="fb">Current emphasis: {rupture ? 'high hoop-stress rupture cue is active.' : oval ? 'bending ovalisation cue is active.' : 'component definition; no failure-theory calculation yet.'}</p>
  </div>;
}
